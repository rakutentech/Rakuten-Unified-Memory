...
#if 1
#define MALLOC 	rum_alloc
#define FREE 	rum_free	
#define CALLOC  rum_calloc
#define REALLOC rum_realloc
#else
...
markblock(volatile void *ptr, size_t size, unsigned bias, int doprint)
{
	...
	// divide the space into chunks of unsigned long
	pl = (unsigned long *)ptr;
	n = size / sizeof(unsigned long);

	// for chunk steps
	for (i=0; i<n; i++) {
		// set the pattern, and write the pattern to the chunk
		val = ((unsigned long)i ^ (unsigned long)bias);
		pl[i] = val;
		//printf("[%ld] = %ld]\n", (unsigned long) (i*sizeof(unsigned long)), val);

		// print a dot every 64 chunks
		if (doprint && (i%64==63)) {
			printf(".");
		}
	}
...
checkblock(volatile void *ptr, size_t size, unsigned bias, int doprint)
{
	...
	for (i=0; i<n; i++) {
		...
		// if the pattern mismatches, ERROR!!
		if (pl[i] != val) {
			if (doprint) {
				printf("\n");
			}
...
test2(void)
{
	...
	for (size = HUGESIZE; (x = (unsigned *)MALLOC(size))==NULL; size = size/2) {
...
test3(void)
{
	...
	while ((tmp = (struct test3*)MALLOC(sizeof(struct test3))) != NULL) {
...
void
test8(void){
	void *old_ptr = NULL, *ptr = NULL;
	int len = 1;

	while((ptr = REALLOC(old_ptr, len)) != NULL){
		old_ptr = ptr;
		len++;
	}
}
...
main(int argc, char *argv[])
{
	...
	const char* path = "/home/paul/RUM/machines.csv";
	rum_init(path);
